...
const validatorErrorHandler = err => {
    let errors = Object.values(err.errors).map( val => val.message.split(".")[1]); 
...
const globalErrorController = (error, req, res, next) => {
    ...
    if(process.env.NODE_ENV === "development") {
        ...
        if(error.name == "SequelizeUniqueConstraintError")  error = duplicateHandler(error) // handling sequelize error for duplicate entry in unique column
        if(error.name == 'SequelizeValidationError')  error = validatorErrorHandler(error) // sequelize validator error
        if(error.name == 'TokenExpiredError')  error = jwtExpiredHandler(error)
        if(error.name == 'JsonWebTokenError')  error = jwtErrorHandler(error)
        if(error.name == 'SequelizeForeignKeyConstraintError') error = foreignKeyErrorHandler(error);
