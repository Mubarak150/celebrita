...
const sendSuccess = (res, status_code, message, data = null, dataKey = 'data', count = null, token = null) => {
...
const create = (model) => asyncErrorHandler(async (req, res, next) => {
...
const getAll = async(req, res, model, {sub_model, alias}, attributes) => {
    const features = new ApiFeatures(model, {sub_model, alias}, req.query).filter().sort().limit_fields();
    (await features.paginate()).includes(sub_model, alias, attributes); 

    let results = await model.findAll(features.queryOptions);

    if(results.length == 0 || !results) sendSuccess(res, 200, "no matching data found");
    else { 
        results = await parse(results); 
        sendSuccess(res, 200, "data retrieved", { results:results, metadata: features.paginationMetadata });
    }
}
...
const getOne = async(req, res, model, {sub_model, alias}, attributes) => {
    const features = new ApiFeatures(model, {sub_model, alias}, req.params).filter().sort().limit_fields().includes(sub_model, alias, attributes); 
...
const update = (model) => asyncErrorHandler(async (req, res, next) => {
    const transaction = await model.sequelize.transaction();
    try {
